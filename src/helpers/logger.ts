...
abstract class BaseTransport implements LogTransport {
  ...
  abstract error(message: string, meta: Record<string, any>): Promise<any> | void;
  ...
  abstract debug(message: string, meta: Record<string, any>): Promise<any> | void;
}
...
class BetterStackTransport extends BaseTransport {
  ...
  constructor(sourceToken: string, ingestingHost?: string, minLevel: LogLevel = LogLevel.INFO) {
    super(minLevel);
...
class Logger {
  ...
  error(message: string, meta: Record<string, any> = {}) {
    ...
    this.transports.forEach(transport => transport.error(message, combinedMeta));
    return this;
  }
  ...
  warn(message: string, meta: Record<string, any> = {}) {
    const combinedMeta = { ...this.context, ...meta };
    this.transports.forEach(transport => transport.warn(message, combinedMeta));
    return this;
  }
  ...
  info(message: string, meta: Record<string, any> = {}) {
    const combinedMeta = { ...this.context, ...meta };
    this.transports.forEach(transport => transport.info(message, combinedMeta));
    return this;
  }
  ...
  debug(message: string, meta: Record<string, any> = {}) {
    const combinedMeta = { ...this.context, ...meta };
    this.transports.forEach(transport => transport.debug(message, combinedMeta));
    return this;
  }
...
export function initLogger() {
  ...
  if (env.BETTERSTACK_SOURCE_TOKEN) {
    ...
    console.warn('BETTERSTACK_SOURCE_TOKEN not provided, BetterStack logging disabled');
  }
